...
def benchmark_bias(bench_df):
    ...
    if close > ema20 > ema50 and change_5d > 0:
        bias = "Bullish"
        message = "US market trend is supportive. Prefer high-conviction long setups."
    elif close < ema20 < ema50 and change_5d < 0:
        bias = "Bearish"
        message = "US market trend is weak. Prefer shorts or stay selective."
    else:
        bias = "Mixed"
        message = "US market is mixed. Prefer only the highest-conviction setups."
...
def render_html(rows, market, perf_log):
    ...
    table_html = pd.DataFrame(rows).to_html(index=False, classes="data-table", escape=False) if rows else "<p>No qualifying setups today.</p>"
    perf_html = perf_log.to_html(index=False, classes="data-table", escape=False) if not perf_log.empty else "<p>No records yet.</p>"
...
<div class="container">
  ...
  <div class="section">
    ...
    <p>
      <b>85+</b> Highest Priority ·
      <b>80–84</b> Medium Priority ·
      <b>75–79</b> Low Priority.
